...
That extension task builds `@librdkafka//:librdkafka`, `@librdkafka//:librdkafka_cpp`, and a lightweight `//:extension_deps_test` shell test that builds `@librdkafka//:librdkafka` and the configured `@librdkafka_deps` aliases as test data. Full standalone librdkafka test binaries are intentionally not enabled on this path yet: linking them against the current boringssl-backed configuration exposes upstream unresolved TLS symbols (`SSL_CTX_use_cert_and_key`, `RAND_priv_bytes`). Envoy is expected to validate the integrated boringssl/zlib-ng configuration first before proposing this pattern upstream to BCR.
